...
#| fig-width: 7
#| fig-height: 7
cites <- do.call(rbind, citations)
cites_year <- as.Date(ISOdate(cites$publication_year, 01, 01))

ggplot2::ggplot(data = data.frame(year = cites_year)) +
  ggplot2::aes(x = year) +
  ggplot2::geom_histogram(binwidth = 365.25, fill = "grey", color = "black") +
  ggplot2::labs(y = "Citations", caption = "Source: OpenAlex.") +
  ggplot2::theme_bw() +
  ggplot2::theme(
...
downloads |>
  ...
  ggplot2::labs(caption = "Source: RStudio CRAN mirror.") +
  ggplot2::theme_bw() +
  ggplot2::theme(
...
downloads <- downloads |> 
  ...
  lapply(
    FUN = function(x) {
      x <- x[-nrow(x), , drop = FALSE] # Remove current month
